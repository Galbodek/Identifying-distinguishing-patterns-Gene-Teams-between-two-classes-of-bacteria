...
REPORT_FILE_PATH = os.getcwd() + "\\report.txt"
LABEL1 = "Animal"
LABEL2 = "Plant"
MIN_SUP = 172 #the minimum MIN_SUP that ran!
...
def create_dataSet(chosen_habitats_table, cog_words_bac_dict):
    ...
    for row in chosen_habitats_table:
        ...
        if cog_words_bac_dict.get(uid) is not None:
            ...
            values = cog_words_bac_dict.get(uid).values() # the values contain the words in specific row in cog_words
            for value in values:
                words = value.split("\t")
                for word in words[1:]: # the first word is the id of the strain and the last word will be just "\t"
                    if word not in  ('', 'X'):
...
if __name__ == '__main__':
    ...
    dataset, labels_and_uids_list = create_dataSet(animals_and_plants_table, cog_words_bac_dict)
